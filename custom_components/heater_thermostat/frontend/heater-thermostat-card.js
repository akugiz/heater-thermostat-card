...
show_step_buttons: "Show side − / + buttons",
...
_sideButtons(type, model) {
if (this._config.show_step_buttons === false) return "";
const colourClass = type === "on" ? "on" : "off";
return `
<div class="side-buttons ${colourClass}">
<button
class="side-step ${colourClass}"
data-threshold="${type}"
data-direction="1"
...
${this._sideButtons("on", model)}
<div class="arc-column">
<svg id="arc" viewBox="0 0 360 132" role="img" aria-label="Heater temperature thresholds">
...
this.shadowRoot.querySelectorAll(".side-step").forEach((button) => {
...
const y = ((event.clientY - rect.top) * 132) / rect.height;
let angle = (Math.atan2(y - 120, x - 180) * 180) / Math.PI;
